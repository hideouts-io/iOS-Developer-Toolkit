...
class LiveLogWindow(QMainWindow):
    ...
    def _copy_raw_with_metadata(self, raw_destination: Path, metadata_destination: Path) -> None:
        if self._raw_file is not None:
            self._raw_file.flush()
        ...
        saved_metadata["raw_sha256"] = sha256_file(raw_destination)
    ...
    def _save_raw_as(self) -> None:
        ...
        try:
            ...
            self._copy_raw_with_metadata(destination_path, metadata_destination)
        except (OSError, LiveLogError) as error:
        ...
        QMessageBox.information(
            self,
            "Raw Capture Saved",
            f"Saved the complete unfiltered byte stream to:\n{destination_path}\n\nMetadata with the raw SHA-256 was saved beside it.",
        )
